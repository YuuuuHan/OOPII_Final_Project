...
public class A1073317_checkpoint7_DemoFrame extends JFrame {
    ...
    private ArrayList<A1073317_checkpoint7_Block> obstacle;
    ...
    private ArrayList<A1073317_checkpoint7_Block> sands;
    //Description : the object to query data.
    private A1073317_checkpoint7_QueryDB querydb;
    //Description : the player object.
    private A1073317_checkpoint7_Player player;
    ...
    private A1073317_checkpoint7_Block[][] map;
    ...
    public A1073317_checkpoint7_DemoFrame(int FWidth, int FHeight,String mapID,int jfScaler, int milliseconds, int algorithm) throws HeadlessException {
        this.FWidth = FWidth;
        ...
        this.obstacle = new ArrayList<A1073317_checkpoint7_Block>();
        this.obstacleDataStructure = new ArrayList<Integer[]>();
        this.sands = new ArrayList<A1073317_checkpoint7_Block>();
        this.sandsDataStructure = new ArrayList<Integer[]>();
        this.algorithm = algorithm;
        this.querydb = new A1073317_checkpoint7_QueryDB(mapID);
        ...
        this.player = new A1073317_checkpoint7_Player("player",jfScaler,SwingConstants.CENTER);
        this.sandsDataStructure = querydb.getSands();
        this.sands = toGUIIdx(sandsDataStructure, SANDTYPE);
        A1073317_checkpoint7_DemoPanel panel = new A1073317_checkpoint7_DemoPanel(player, obstacle,jfScaler, sands);
        this.add(panel);
        ...
        panel.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                int endX=ReleasedX-PressedX;
                int endY=ReleasedY-PressedY;

                if(!player.getOnClick()){                        
                        panel.setCenterX(panel.getCenterX()-endX);
                        panel.setCenterY(panel.getCenterY()-endY);
                        repaint();
                }

                if(player.getOnClick()){
                    if(!player.getMoving()){
                        A1073317_checkpoint7_PlayerMovement move = new A1073317_checkpoint7_PlayerMovement(player,panel,ReleasedX,ReleasedY,milliseconds,algorithm,map);
    ...
    public ArrayList toGUIIdx(ArrayList<Integer[]> data, String blockType){
        ArrayList<A1073317_checkpoint7_Block> dataGui = new ArrayList<A1073317_checkpoint7_Block>();
        for(Integer[] x : data){
            if(blockType == "obstacle"){
                dataGui.add( new A1073317_checkpoint7_Block(x[1]-1, x[0]-1,blockType, obstacleImg.get(x[2]), 100000));
            }else if(blockType == "sand"){
                dataGui.add( new A1073317_checkpoint7_Block(x[1]-1, x[0]-1,blockType, new ImageIcon("./Resource/sand.jpg").getImage(), SANDWEIGHT));
            }
        }
        return dataGui;
    }
    ...
    public A1073317_checkpoint7_Block[][] createMap(int height,int width){
        A1073317_checkpoint7_Block[][] map = new A1073317_checkpoint7_Block[width][height];
        for(A1073317_checkpoint7_Block block : obstacle){
            map[block.getX()][block.getY()] = block;
        }
        for(A1073317_checkpoint7_Block block : sands){
            map[block.getX()][block.getY()] = block;
        }
        for(int x = 0; x<width; x++){
            for(int y = 0; y<height; y++){
                if(map[x][y] == null){
                    map[x][y] = new A1073317_checkpoint7_Block(x,y,"space",null, SAPCEWEIGHT);
                }
            }
        }
        return map;
